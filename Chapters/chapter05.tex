...
We would like to use the Algebraic Group Model in the isogeny setting, since it's very helpful with proofs. However we don't want to restrict too much the power of the adversary. We will thus define a new model, analogous to AGM; we also think that our model could be equivalent to the plain model.
...
\begin{proof}
    ...
    which proves indistinguishability, provided that the encryption scheme is $\indcpa$ and the CSIDH problem is hard.\todo{There must be some coefficients in this...}
    
\end{proof}


\section{The generality of EI model}
...
The conclusion of this chapter, and the main takeaway of this tesis, is that while it would be good to find an efficient sampling algorithm,\todo{actually, why? References?} if we instead get convinced that the problem is hard we could have gained a very useful tool for UC proofs, that in particular avoids unnecessary computations in many protocols.
